...
- apart from <kbd>Tab</kbd> for indentation, <kbd>Ctrl</kbd>+<kbd>S</kbd> for saving and simple autocompletion via <kbd>Ctrl</kbd>+<kbd>Space</kbd>, it lacks any comfort and visual beauty: no syntax highlighting or validation (no Ace editor, jslint, ...); the browser console, however, proved sufficient for debugging
